...
def test_offsets():
    ...
    assert np.shape(offsets) == (3,)
    assert np.array_equal(offsets, np.array([.1, .2, .3]))

    _, offsets = weight_initializer.init(3, 1)
    assert np.shape(offsets) == (1,)
    assert np.array_equal(offsets, np.array([.4]))


def test_wrong_weights_shape():
    weight_initializer = FixedInitializer(([[1]]), [0, 0])
    with pytest.raises(ValueError, match=re.escape("Expected weights shape (2, 2), but got (1,)")):
        weight_initializer.init(2, 2)


def test_wrong_offsets_shape():
    weight_initializer = FixedInitializer(([[0], [0]],), [1])
    with pytest.raises(ValueError, match=re.escape("Expected offsets shape (2,), but got ()")):
